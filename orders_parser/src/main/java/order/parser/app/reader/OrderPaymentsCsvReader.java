...
public class OrderPaymentsCsvReader implements Runnable{

    private final BlockingQueue<OrderPaymentLine> blockingQueue;

    private final String path;

    public OrderPaymentsCsvReader(BlockingQueue<OrderPaymentLine> blockingQueue, String path) {
    ...
    @Override
    public void run() {
        Path csvFilePath = FileSystems.getDefault().getPath(this.path);
